...
public class ConfigProvider(BossesHaveGpCoinsConfig config) : IConfigEditorConfigProvider
{
    public IEnumerable<ConfigEditorConfigRegistration> GetConfigs()
    {
        ...
        metadata.Name,
        config,
        Path.Combine("user", "mods", "acidphantasm-bosseshavegpcoins", "config.json")
        );
    }
}